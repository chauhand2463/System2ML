...
class ResourceLimits:
    def __init__(
        ...
        max_duration_ms: float = 3600000.0,
        max_step_duration_ms: float = 600000.0,
    ):
        self.max_cost_usd = max_cost_usd
        self.max_carbon_kg = max_carbon_kg
        self.max_memory_mb = max_memory_mb
        self.max_duration_ms = max_duration_ms
        self.max_step_duration_ms = max_step_duration_ms
...
class PipelineExecutor:
    ...
    def _check_limits(self, metrics: ExecutionMetrics) -> None:
        ...
        if metrics.duration_ms > self.limits.max_duration_ms:
            raise ResourceLimitExceeded(
                "duration", self.limits.max_duration_ms, metrics.duration_ms
            )
    ...
    def execute(
        self, pipeline_id: str, steps: list, limits: Optional[ResourceLimits] = None
    ) -> dict:
        ...
        try:
            ...
            with self._lock:
                if execution_id in self._running_executions:
                    self._running_executions[execution_id]["status"] = ExecutionStatus(
